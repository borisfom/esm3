...
class Engine():
    ...
    def build(self, onnx_path,
              profiles=[], fp16=False, bf16=False, tf32=False,
...
class TRTWrapper(torch.nn.Module):
    ...
    def attach(self, exp):
        ...
        self.output_names = exp.output_names

    def can_handle(self, **args):
        return self.exp_wrapper.can_handle(**args)
    ...
    def forward(self, **args):
        try:
            if self.engine is not None:
                if self.can_handle(**args):
                    print(f"Running {self.engine_path}...")
                ...
                print(f"Running {self.onnx_path}...")
        ...
        return self.exp_wrapper.forward(**args)
    ...
    def forward_trt(self, trt_inputs):
        ...
        ret = self.engine.infer(trt_inputs, stream.cuda_stream, use_cuda_graph=False)
        ret = list(ret.values())
        for r in ret:
    ...
    def onnx_export(self, input_example,
        ...
        L_.info(f"Exporting to ONNX, dynamic axes: {dynamic_axes}")
        if onnx_registry is not None:
            ops = torch.onnx.ExportOptions(dynamic_shapes=True)
            #                                           onnx_registry=onnx_registry)
            export_output = torch.onnx.dynamo_export(
                self.exp_wrapper.get_export_obj(),
                *input_example,
                # export_options=ops,
            )
            export_output.save(self.onnx_path)
        else:    
            torch.onnx.export(self.exp_wrapper.get_export_obj(),
                          input_example,
                          self.onnx_path,
                          verbose=verbose,
